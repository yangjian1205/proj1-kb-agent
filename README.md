...
这里说人话就是给公司的规章制度配备了一个不会说瞎话的图书管理员，你问什么他就依据已有的规章制度回答，绝对不会瞎说，不知道就是不知道 

---

## 这个项目解决什么问题

举个例子，新入职的员工不清楚公司的考勤制度，可以直接问知识库，他会依据已有的制度回答。并且会标注回答出资哪里，你可以照着原文对比。如果资料里真没有的（比如"公司有健身房吗"），它直接说"知识库未找到相关信息"，而不是编一个听起来很合理的答案把你糊弄过去。 
...
混合检索（BM25 + 向量）→ RRF 融合 → 三档拒答 → 引用溯源。自建 50 题评测集上块级 hit@5 达 **100%**（纯向量）/ **93.3%**（纯 BM25）/ **97.8%**（RRF 融合），拒答端到端 **50/50**、可答问题误拒 **0/45**。 
